...
function renderGame(){
 ...
 $('#vsColumn').classList.toggle('hidden',!vs);$('#roundHistory').classList.toggle('hidden',vs);if(vs)renderVersus();else{clearBoardHighlights();renderRoundHistory()}renderTurnDarts();
}
...
function renderTurnDarts(){const vs=active.gameType==='CRICKET_VS',r=vs?active.currentRound:normalRound(),player=vs?active.currentPlayer:1,ds=active.darts.filter(d=>d.roundNumber===r&&(d.player||1)===player&&!d.autoBustFill);$('#turnDarts').innerHTML=[1,2,3].map(n=>{const d=ds.find(x=>x.dartNumberInRound===n),detail=d?`${hitLabel(d)}${(active.gameType==='CRICKET'||vs)&&d.marks?` · ${d.marks}M`:''}`:'—';return`<div class="turn-dart ${d?'':'empty'}"><small>${n}投目</small><b>${detail}</b></div>`}).join('')}
function clearBoardHighlights(){$$('#board .wedge').forEach(w=>w.classList.remove('score-player-1','score-player-2'))}
function highlightScoringTargets(state){clearBoardHighlights();CRICKET_TARGETS.forEach(t=>{const cls=state.marks[0][t]>=3&&state.marks[1][t]<3?'score-player-1':state.marks[1][t]>=3&&state.marks[0][t]<3?'score-player-2':null;if(!cls)return;const selector=t==='BULL'?'#board [data-type="OUTER_BULL"],#board [data-type="INNER_BULL"]':`#board [data-number="${t}"]`; $$(selector).forEach(w=>w.classList.add(cls))})}
function renderVersus(){
 const state=cricketState(active);highlightScoringTargets(state);const rows=CRICKET_TARGETS.map(t=>`<div class="cricket-row"><div class="cricket-cell">${markSvg(state.marks[0][t],1)}</div><div class="cricket-cell target">${t}</div><div class="cricket-cell">${markSvg(state.marks[1][t],2)}</div></div>`).join('');
 ...
 $('#vsScoreboard').innerHTML=`<div class="cricket-table">${rows}</div><div class="vs-log"><div class="vs-log-title">投球履歴（ラウンド・選手・投目）</div>${log||'<div class="empty">まだ投球がありません</div>'}</div>`;
 $('#vsStatusBottom').innerHTML=`<div class="bottom-player ${active.currentPlayer===1?'active':''}"><small>PLAYER 1</small><b>${state.scores[0]}</b></div><div class="bottom-round"><small>ROUND</small><b>R${active.currentRound}</b><span>${currentTurnDarts().length+1}本目</span></div><div class="bottom-player ${active.currentPlayer===2?'active':''}"><small>PLAYER 2</small><b>${state.scores[1]}</b></div>`;
}
